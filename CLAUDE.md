...
`typescript` is an explicit devDependency: `expo-module build` shells out to `npx tsc`, which needs a local `typescript` install to resolve the actual compiler. Without it declared directly, `tsc` only worked here because it was hoisted incidentally from sub-dependencies (`eslint-config-universe`, `ts-jest`) — not guaranteed by any dependency tree, and confirmed to fail for real consumers: `npx tsc` resolved an unrelated abandoned `tsc` package from the registry instead, breaking `prepare` (and therefore the entire git-based install) on a fresh clone.